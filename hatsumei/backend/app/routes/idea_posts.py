...
from app import db
from datetime import datetime, timezone  # datetime モジュールをインポート
...
def create_idea_post():
    ...
    print(f"Received data: {data}")  # 受け取ったデータを確認

    try:
        # `user_id` が存在しない場合はエラーを返す
        ...
        db.session.rollback()  # 何か問題があればロールバック
