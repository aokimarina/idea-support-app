...
//   const [likes, setLikes] = useState(idea_posts.map(() => 0)); // 各投稿のいいね数を管理
...
import { fetchIdeaPosts } from "../../server/ideaPost"; // fetchIdeaPosts をインポート
import { IdeaPostType } from "../../../types/types"; // IdeaPostType をインポート

const AllPostPage = () => {
  const { height, width } = useWindowSize();
  const [likes, setLikes] = useState<number[]>([]);
  const [posts, setPosts] = useState<IdeaPostType[]>([]); // 投稿データを管理する state
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setPosts(ideaPosts);
        setLikes(ideaPosts.map(() => 0)); // 投稿数に合わせて likes を初期化
      } catch (error) {
